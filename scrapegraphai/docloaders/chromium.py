...
class ChromiumLoader(BaseLoader):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Pull out special loader kwargs that are NOT meant for playwright.launch()
        self.click_selectors = kwargs.pop("click_selectors", None)  # list[str] or None
        self.max_clicks = kwargs.pop("max_clicks", kwargs.get("max_clicks", 10))
        self.item_selector = kwargs.pop("item_selector", None)  # CSS selector to detect new items
        self.wait_after_click = kwargs.pop("wait_after_click", kwargs.get("wait_after_click", 1000))  # ms
        ...
        while attempt < self.retry_limit:
            try:
                async with async_playwright() as p, async_timeout.timeout(self.timeout):
                    ...
                    # ---- START: click/scroll loop to press "Load More" or trigger infinite scroll ----
                    click_selectors = self.click_selectors or [
                        "div.LoadMoreButton_btnWrapper__CtkKX",
                        "div.LoadMoreButton_btnWrapper__CtkKX > span",
                        "text='Load More'",
                        "text='Load More'"
                    ]
                    max_clicks = int(self.max_clicks or 10)
                    item_selector = self.item_selector or "div.NewsList_newsListItemWrap__XovMP"
                    wait_after_click = int(self.wait_after_click or 1000) / 1000.0  # convert ms -> s
                    ...
                    try:
                        ...
                        while clicks < max_clicks:
                            ...
                            for sel in click_selectors:
                                try:
                                    ...
                                    if el:
                                        # check visibility and enabled if available
                                        try:
                            ...
                            # allow JS/network activity
                            await asyncio.sleep(wait_after_click)

                            # check if new items loaded (if selector provided)
                            if item_selector:
                        ...
                        logger.debug(f"Pagination/click loop issue (non-fatal): {e}")
                    # ---- END: click/scroll loop ----
